...
input string      inpTradeComments     =  "Holy Grail v8";  //Trade comment
...
if (CurrentCondition==ENUM_CONDITION_SHORT_PULL_BACK && FirstTrade2==true)
   {
      TriggerBar= FindTriggerBar();
      StopLoss= GetSwingHigh(TriggerBar);
       TradeEnterPrice = iLow (Symbol(),Period(),TriggerBar+1);
       DrawStopLoss(StopLoss);
       DrawEntryPrice(TradeEnterPrice);
       AdxCross=FindADXCrossUP (400);
...
double FindPreviousLow (int cross)
   ...
   double newBar;
      for (int k=2;k<cross+1;k++)
            ...
            {
            newBar= iLow (Symbol(),Period(),k);
            if (newBar>LowBar)LowBar=newBar;
...
void waitingForShortEntry ()
 ...
 double static ADX3 = iADX(Symbol(), Period() , 14, PRICE_CLOSE, MODE_MAIN, 1);
   ...
   if (iHigh(Symbol(),Period(),1)>StopLoss)
      {
      StopLoss=iHigh(Symbol(),Period(),1);
      DrawStopLoss(StopLoss);
      }   
   
   if (StopLoss-TradeEnterPrice!=0)  RiskReward= (TradeEnterPrice-TargetLow) / (StopLoss-TradeEnterPrice);
   Comment ("\nl Risk Reward on trade = " +NormalizeDouble(RiskReward,1));
   if (RiskReward<inpMinRiskReward)
      {
      CurrentCondition=ENUM_CONDITION_NO_CONDITION;
   ...
   if (iClose(Symbol(),Period(),1)<TradeEnterPrice && AccountFreeMargin()>inpCashMin)
      ...
      double tp= TradeEnterPrice-TargetLow;
      double stpl= StopLoss- TradeEnterPrice;
      ...
      TradeOne= orderOpen(ORDER_TYPE_SELL,stpl,tp*inpTPfactor,Lots,inpTradeComments,inpMagicNumber);
           
      if (TradeOne>0) CurrentCondition=ENUM_CONDITION_IN_A_SHORT_HEADING_FOR_TARGET; else 
      
      {Print("Error "+GetLastError()+" tp=  "+ tp*inpTPfactor+" SL= "+stpl);
      CurrentCondition=ENUM_CONDITION_WAITING_FOR_LONG_ENTRY;
      }
      return;
      }
