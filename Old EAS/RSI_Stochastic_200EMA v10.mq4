...
input double      inpMultiplier        = 1.2;// input multiplier effect to increas traid volume on each trade
...
input double   enterTP           =  0.2; // Enter first take profit
input int   entMaxtrades        =  6;// Enter the maximum number of trades
...
input string   inpTradeComments  =  "RSI STOCHASTIC 200 V10";  //Trade comment
...
Docket= CheckLongTrade (higherHigh,lowerLow,multiplier,XmarketTurnCounter);}
  ...
  
 {
      ...
      marketTurnCounter=marketTurnCounter+1;
       ...
       multiplier=multiplier*inpMultiplier; 
     ...
     RefreshRates(); 
      ...
      if (minstoplevel>spread) stopvalue =minstoplevel*Point; 
       ...
       openPrice = NormalizeDouble (SymbolInfoDouble(Symbol(), SYMBOL_BID), Digits());
        ...
        takeProfit= NormalizeDouble ((stopLossPrice-openPrice)+(ATR*ATRFactor*points),Digits());
          ...
          if ( (stomain>stosig) && (XmarketTurnCounter< inpMarketTurnCounter)) 
                 {
                       ...
                       double multiplier=multiplier*inpMultiplier; 
                       double volume = NormalizeDouble(inpOrderSize,Digits());
                       RefreshRates(); 
                       ...
                       if (minstoplevel>spread) stopvalue =minstoplevel*Point; 
                        ...
                        int count = 0;
                         ...
                         takeProfit= NormalizeDouble ((openPrice-stopLossPrice)+(ATR*ATRFactor*points),Digits());
