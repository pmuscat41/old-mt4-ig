...
input double               inpEMAExit=200;// Enter Exponential Moving Average to exit trade
...
if ((completed==true) && (tradecontinuously==true)) 
      ...
      {  
         ...
         OrderSelect (Docket,SELECT_BY_TICKET);
            if (OrderCloseTime()==0) 
               { Alert ("Actively managing the trade");
               
               ////////// trade management
                
                
                //CheckForClose (Docket);
...
if (Pivot3<Pivot2)
   if (Pivot1>Pivot2)
      if (iClose(Symbol(),Period(),0)  >twohundrMA)
         longtrend=true;
...
datetime lineend;
             ...
             linestart= iTime(Symbol(),PERIOD_H4,30);
             lineend = iTime(Symbol(),PERIOD_H4,0);
            ObjectDelete(0,"P1");
            ObjectCreate ("P1",OBJ_TREND,0,
            linestart,Pivot1, 
            lineend,Pivot1);
            ObjectSetInteger(0,"P1",OBJPROP_COLOR,clrAqua);
            ...
            ObjectSetInteger(0, "P1",OBJPROP_RAY_RIGHT, false);
             ...
             linestart= iTime(Symbol(),PERIOD_H4,60);
             lineend = iTime(Symbol(),PERIOD_H4,31);
             
            ObjectDelete(0,"P2");
            ObjectCreate ("P2",OBJ_TREND,0,
            linestart,Pivot2, 
            lineend,Pivot2);
            ObjectSetInteger(0,"P2",OBJPROP_COLOR,clrAqua);
            ...
            ObjectSetInteger(0, "P2",OBJPROP_RAY_RIGHT, false);
             ...
             linestart= iTime(Symbol(),PERIOD_H4,90);
             lineend = iTime(Symbol(),PERIOD_H4,61);
            ObjectDelete(0,"P3"); 
            ObjectCreate ("P3",OBJ_TREND,0,
            linestart,Pivot3, 
            lineend,Pivot3);
            ObjectSetInteger(0,"P3",OBJPROP_COLOR,clrAqua);
...
void CheckForClose (int Docket)

{


                  bool Exitshort,Exitlong;
                  double EMAExit= iMA(Symbol(),Period(),inpEMAExit,0,MODE_SMA,PRICE_CLOSE,0);
                  Exitlong=false;
                  Exitshort=false;
                  ...
                  if (OrderType()== ORDER_TYPE_BUY)
                     if (Bid<EMAExit) Exitlong=true;
                   
                   if (OrderType()== ORDER_TYPE_SELL)
                     if (Ask >EMAExit) Exitshort=true;  
