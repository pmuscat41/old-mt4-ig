...
input string   inpTradeComments  =  "Connors PowerZones Strategy v2";  //Trade comment
...
//// -----------------subsequent entries
      ...
      if((totalTicket>0)&&(oversold2) && (direction>0) && (totalTicket<inpTotalOrders)  && (OrderOk==true))
       {
      ticket   =  orderOpen(ORDER_TYPE_BUY, inpStopLoss,inpTakeProfit);
      lastTicket = ticket;
      previousDirection= direction;
      ...
      if((totalTicket>0)&&(overbough2) &&(direction<0) && (totalTicket<inpTotalOrders) && (OrderOk==true))
        {
         ticket  =  orderOpen(ORDER_TYPE_SELL, inpStopLoss,inpTakeProfit);
         lastTicket= ticket;
         previousDirection=direction;
