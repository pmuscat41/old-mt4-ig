...
input double   inpTotalOrders    =  2; //Set total orders to open
input string   inpTradeComments  =  "Connors PowerZones Strategy v7";  //Trade comment
...
//Bar 0 is currently open, bar 1 is the most recent closed bar and bar 2 is bar before the
   double rsi     =  iRSI(Symbol(),PERIOD_D1,inpRSIPeriods, inpRSIPrice,0);
   double rsiYest     =  iRSI(Symbol(),PERIOD_D1,inpRSIPeriods, inpRSIPrice,1);
...
Comment ("\n first position opened at 30/70, second position opened at 35,75"); 
 ...
 if (DayOfWeek()=="Sunday") OrderOk=false;
   else OrderOk==true;
 
///// -------------------- first entry
   
      if((totalTicket==0) &&(oversold) && (direction>0) && (totalTicket<inpTotalOrders)  && (OrderOk==true)&&(Sellnow==true))
       {
      ...
      if((totalTicket==0)&&(overbought) &&(direction<0) && (totalTicket<inpTotalOrders) && (OrderOk==true)&& (Buynow==true))
        {
