...
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
/// V7 ADDING PRICE TO TP, SL AND ENTRY PRICE
/// v7 added trailing stop 
...
input ENUM_EXIT_STRATEGY             InpExitStrategy = EXIT_DYNAMIC_STOP;//Enter exit strategy
input int                        InpCandlesToCancel = 100;// Enter number of candles to deactivate EA

input int                        InputMagicNumber=202020; // Magic number
input string                      InpTradeComment= "Trendline Violation v7"; //Trade Comment
...
int OnInit()
  {
     ...
     ObjectSetText("TLT", " Entry Price= "+NormalizeDouble(ObjectGetValueByShift("TL",1),Digits), 8, "Arial", clrAntiqueWhite);
      }
...
// Stoploss 
      ...
      ObjectSetText("SLT", " Stop Loss= "+NormalizeDouble (ObjectGetValueByShift("SL",1),Digits), 8, "Arial", clrRed);
      }
...
// Take Profit 
     ...
     ObjectSetText("TPT", " Take Profit = "+ NormalizeDouble (ObjectGetValueByShift("TP",1),Digits), 8, "Arial", clrGreen);
      }
...
ObjectSetText("TLT", " Entry Price= "+NormalizeDouble(ObjectGetValueByShift("TL",1),Digits), 8, "Arial", clrAntiqueWhite);
    ...
    if (InpDirection==PM_TRADE_LONG)TrailStopSize= EntryPrice-StopLoss;
    if (InpDirection==PM_TRADE_SHORT)TrailStopSize= StopLoss-EntryPrice;
...
if (InpExitStrategy==EXIT_TRAILING_STOP)TrailingStop(InputMagicNumber,ticket,TrailStopSize);
