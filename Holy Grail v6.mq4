...
{
Comment ("in a long trade managing trade");

// exit if trades closed
...
double Mstop =NormalizeDouble(MAStop,Digits);
   ...
   OrderSelect(TradeTwo,SELECT_BY_TICKET);
   ...
   if (OrderCloseTime()!=0) 
        ...
        { 
        er=OrderModify(TradeTwo,OrderOpenPrice(),Mstop,OrderTakeProfit(),0,clrRed);
        Comment ("---trailing stops------------");
 ...
 if (CurrentCondition==ENUM_CONDITION_WAITING_FOR_LONG_ENTRY)
   
  {
   ...
   if (iClose(Symbol(),Period(),1)>TradeEnterPrice)
      ...
      if (TradeTwo>0) CurrentCondition=ENUM_CONDITION_IN_A_LONG_HEADING_FOR_TARGET; else 
