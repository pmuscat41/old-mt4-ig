...
////v4 full working version - TRYING TO ADD ichimuuko cloud stops
...
input string   inpTradeComments  =  "Macd divergence with camarilla v5";  //Trade comment
...
if ((completed==true) && (tradecontinuously==true)) 

      {
         ...
         OrderSelect (Docket,SELECT_BY_TICKET);
            if (OrderCloseTime()==0) 
               { Alert ("IN A TRADE          IN   A   T R A D E  ");
      ...
      OrderSelect (Ticket, SELECT_BY_TICKET);
            ...
            if (OrderMagicNumber()==inpMagicNumber)
               if (OrderType()==ORDER_TYPE_BUY)
                ...
                { 
                        newstoploss= iIchimoku(Symbol(),PERIOD_D1, 9,26,52,MODE_KIJUNSEN,0);
                        
                        Alert ("new stop= "+newstoploss);
